...
{
	// not yet implemented
	rq -> curr -> task_time_slice = rq -> curr -> weighted_time_slice;
...
static struct task_struct *pick_next_task_weighted_rr(struct rq *rq)
{
	...
	next = list_first_entry(&(rq -> weighted_rr.queue) , struct task_struct , weighted_rr_list_item);	
	return (next);
...
static void task_tick_weighted_rr(struct rq *rq, struct task_struct *p,int queued)
{
	...
	if(p ->  task_time_slice == 0)
	{
		...
		//curr = p;
		 ...
		 if (curr->weighted_rr_list_item.prev != curr->weighted_rr_list_item.next){
